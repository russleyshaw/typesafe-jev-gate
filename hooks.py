...
def jev_gate(tool_name: str, args: dict[str, Any], **kwargs: Any) -> dict[str, str] | None:
    ...
    try:
        answers = evaluate_tool(tool_name, args)
        ...
        score = risk(answers)
        audit({"tool": tool_name, "outcome": "evaluated", "risk": score, "paid_tool": tool_name in PAID_TOOLS})
...
def route_turn(user_message: str = "", **kwargs: Any) -> str | None:
    ...
    if len(text) < 160 and not any(marker in text.lower() for marker in markers):
        return None
    try:
        choice = route_request(text).get("route", {}).get("choice")
        if not choice:
            return None
